...
cursor.execute("select count(*) from ums_jobprivileges;")
if cursor.fetchone()[0] != len(privileges):
    insert = 'insert into ums_jobprivileges (code_name, name) values' 
    for i in privileges:
...
cursor = connection.cursor()
cursor.execute("select count(*) from ums_jobtype;")
if cursor.fetchone()[0] != len(types):
    insert = 'insert into ums_jobtype (name, color) values' 
    for i in types:
        insert += ' (\'' + i + '\', \'' + types[i] + '\'),'
    insert = insert[:-1]
    insert += ';'
...
cursor = connection.cursor()
cursor.execute("select count(*) from ums_jobstatus;")
if cursor.fetchone()[0] != len(statuses):
    insert = 'insert into ums_jobtype (name, description) values' 
    for i in statuses:
        insert += ' (\'' + i + '\', \'' + statuses[i] + '\'),'
    insert = insert[:-1]
    insert += ';'
