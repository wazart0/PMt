create temp table availability as (
	...
	selected_availability as (
		select 
			...
			graph_engine_edge.source_node as user,
		...
		inner join pmt_calendar_availability on graph_engine_edge.target_node = pmt_calendar_availability.id
		where 
			graph_engine_edge.source_node in (select * from users)
		and
