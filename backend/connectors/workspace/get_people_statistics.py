...
if __name__ == "__main__":
    ...
    for spreadsheet in spreadsheets:
        ...
        for issue_key in get_lastly_closed_issues(jira, spreadsheet):
            ...
            if issue_key not in ['PP-986', 'PP-984', 'PP-1114', 'PP-1441', 'PP-1442', 'PP-1443', 'PP-1444', 'PP-1447', 'PP-1446', 'PP-1445', 'PP-1446', 'PP-1447', 'PP-1448', 'PP-1449', 'PP-1500']:
                print(issue_key)
                if estimation[jira_keys.index(issue_key)] != '' and (len(duration) <= jira_keys.index(issue_key) or duration[jira_keys.index(issue_key)] == '' or not isclose(float(duration[jira_keys.index(issue_key)]), value, abs_tol=0.01)):
                    ...
                    start = get_issue_first_transition_date(issue_CL, 'In Progress').strftime("%b %d, %Y %H:%M:%S")
                    finish = get_issue_last_transition_date(issue_CL, 'In Review').strftime("%b %d, %Y %H:%M:%S")
